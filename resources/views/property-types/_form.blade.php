...
<div class="bg-white rounded-xl border border-slate-200 p-4 space-y-4">
    <div>
        ...
        <input id="name" name="name" type="text" required value="{{ old('name', $type?->name) }}" class="{{ $input }}">
    </div>
    ...
    <div>
        ...
        <input id="icon" name="icon" type="text" value="{{ old('icon', $type?->icon) }}"
               placeholder="house" class="{{ $input }}">
    </div>
